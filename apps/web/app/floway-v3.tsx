...
import { stationTitle } from './lib/station-label';
...
import { useStationBrands } from './lib/station-brands';
import { distinguerTitres, regrouperParLieu } from './lib/station-list';
import { cleIncident, nonAcquittes } from './lib/alerts';
...
export default function FlowayV3(){
 ...
 const { vehicleConfirmed, vehicle:storeVehicle, trip:storeTrip, setTrip:setStoreTrip, lastRoute, setLastRoute, hydrated:storeHydrated, favoriteRoutes, toggleFavoriteRoute, acknowledgedAlerts, acknowledgeAlerts, pruneAcknowledgedAlerts } = useFlowayStore();
 ...
 const listedStations=useMemo(()=>{
  ...
  const classe=[...eligible].sort((a,b)=>rang(a)-rang(b));
  // Le classement dispersait les stations d'une meme commune — Vierzon,
  // Vierzon, Theillay, Salbris, puis Vierzon a nouveau. Revoir plus bas une
  // commune deja vue se lit comme une erreur, meme quand le score le
  // justifie. On rassemble donc chaque commune au rang de sa meilleure
  // station, sans changer l'ordre relatif du reste.
  return regrouperParLieu(classe,(s:Station)=>s.city||'') as Station[];
 },[eligible,plannedById,stationOrder]);

 // Trois cartes portaient le titre « Vierzon » : sans enseigne au flux et sans
 // adresse nommant un lieu, le titre retombe sur la commune. La voie ne
 // complete le titre que la ou il se repete.
 const titresListe=useMemo(()=>{
  const visibles=listedStations.slice(0,24);
  return distinguerTitres(visibles.map(s=>({id:String(s.id),titre:stationTitle(s),adresse:s.address})));
 },[listedStations]);
 ...
 // Enseignes : absentes du flux du ministere, elles viennent de TomTom et
 // coutent un appel par station. On les demande donc pour les seules etapes du
 // voyage et le meilleur arret — ce que l'utilisateur lit vraiment — et jamais
 // pour la liste entiere.
 const pointsEnseigne=useMemo(()=>{
  const vus=new Set<string>();const out:{id:string;lat?:number;lon?:number}[]=[];
  for(const e of journey.steps){const st=e.station as Station;const id=String(st.id);
   if(!vus.has(id)&&st.lat!=null&&st.lon!=null){vus.add(id);out.push({id,lat:st.lat,lon:st.lon})}}
  if(best&&best.lat!=null&&best.lon!=null&&!vus.has(String(best.id)))out.push({id:String(best.id),lat:best.lat,lon:best.lon});
  return out;
 },[journey,best]);
 const {brands:enseignes}=useStationBrands(pointsEnseigne);
 /** Enseigne trouvee pour une station, ou null. Rien n'est devine. */
 const enseigneDe=useCallback((s?:Station|null)=>(s?enseignes[String(s.id)]?.brand||s.brand||null:null),[enseignes]);
 ...
 // La pastille comptait des incidents que rien ne permettait d'eteindre, et le
 // panneau qu'elle ouvrait ne disait pas lesquels. Elle ne compte desormais que
 // ce qui n'a pas ete acquitte.
 const alertesOuvertes=useMemo(()=>nonAcquittes(nearbyIncidents,acknowledgedAlerts),[nearbyIncidents,acknowledgedAlerts]);
 // Un incident resolu ne doit pas rester acquitte pour toujours : s'il
 // revenait au meme endroit plus tard, il ne serait jamais signale.
 useEffect(()=>{
  if(!nearbyIncidents.length)return;
  pruneAcknowledgedAlerts(nearbyIncidents.map(i=>cleIncident(i)).filter(Boolean) as string[]);
 },[nearbyIncidents,pruneAcknowledgedAlerts]);
 const acquitterAlertes=useCallback(()=>{
  const cles=alertesOuvertes.map(i=>cleIncident(i)).filter(Boolean) as string[];
  acknowledgeAlerts(cles);
  const sansCle=alertesOuvertes.length-cles.length;
  notify(sansCle?`${cles.length} alerte(s) marquée(s) comme lue(s). ${sansCle} sans identifiant stable reste affichée.`:'Alertes marquées comme lues.');
 },[alertesOuvertes,acknowledgeAlerts]);
 ...
 return <main className="v3app">
  <header className="v3top"><button className="v3icon" onClick={()=>setPanel('menu')} aria-label="Menu Floway">☰</button><div className="v3brand"><span>≋</span><strong>Floway</strong><small>CHAQUE PAUSE COMPTE</small></div><div className="v3status"><button onClick={()=>setPanel('alerts')} aria-label={alertesOuvertes.length?`${alertesOuvertes.length} alerte${alertesOuvertes.length>1?'s':''} à proximité`:'Alertes du trajet'}>🔔{alertesOuvertes.length>0&&<b>{alertesOuvertes.length}</b>}</button><span className={route?.traffic?.live?'trafficLive':'trafficOff'}>● {trafficLabel}</span><span>🌦 Météo à connecter</span></div></header>
  ...
  <section className="v3stations" id="v3stations"><div className="v3panelHead"><div><span>STATIONS DEVANT VOUS</span><h2>{stationOrder==='classement'?'Classées par Floway':'Dans l’ordre du trajet'}</h2></div><StationOrderSwitch value={stationOrder} onChange={setStationOrder}/><select value={fuel} onChange={e=>{setFuel(e.target.value);void loadRoute(origin,destination,e.target.value,departure)}}><option>Gazole</option><option>SP95-E10</option><option>SP98</option><option>E85</option></select></div><RoutePriceRibbon summary={priceSummary}/><div className="v3cards">{listedStations.slice(0,24).map((s,index)=><button key={s.id} className={s.highway?'v3highway':undefined} onClick={()=>setSelected(s)}><span>{stationOrder==='classement'?<b className="v3rank">{index+1}</b>:serviceIcon(s.serviceCategories?.[0]||'')}</span><div><strong>{s.highway&&<i className="v3highwayTag">A</i>}{titresListe.get(String(s.id))||stationTitle(s)}</strong><small>{[s.highway?'sur autoroute':s.city,`${Math.max(0,Math.round(s.distanceKm-currentKm))} km devant`,clock(s)].filter(Boolean).join(' · ')}</small><em>{(s.serviceCategories||[]).slice(0,3).join(' · ')}</em></div><b>{typeof s.price==='number'&&<u className="v3price">{s.price.toFixed(3)} €/L</u>}<span>{crowd(s.waitMin).icon} {crowd(s.waitMin).label}</span></b></button>)}</div></section>
  ...
  <ActionSheet panel={panel} onPanel={setPanel} onPickRoute={playRoute} onScrollToStations={scrollToStations} onNotify={notify}
   alerts={alertesOuvertes} alertsConnected={safetyConnected} onAcknowledgeAlerts={acquitterAlertes}/>
 <Toast message={toast}/>
